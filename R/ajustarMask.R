...
dir.create("Resultados_plantas_VaraiablesBaja/mascara/")
...
binarios <- list.files("Resultados_plantas_VaraiablesBaja", 
...
nuevoExt<-lapply(binarios, function(x){
  ...
  final <- mask*sp_map
  outname <- paste0("Resultados_plantas_VaraiablesBaja/mascara/", basename(x))
